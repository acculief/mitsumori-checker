...
const gl = (slug: string, label: string) => (
  <Link href={`/guide/${slug}/`} className="text-primary hover:underline">{label}</Link>
);

const faqItems: { q: string; a: ReactNode }[] = [
  {
    q: "車検の費用相場はいくらですか？",
    a: <>法定費用（{gl("jibaiseki", "自賠責保険")}＋{gl("weight-tax", "重量税")}＋{gl("stamp-fee", "印紙代")}）＋{gl("inspection-fee", "車検基本料")}の合計で、軽自動車で約4〜7万円、小型車で約5〜9万円、中型車で約6〜11万円、大型車で約7〜13万円が一般的な相場です。追加のメンテナンス・部品交換が発生すると、さらに費用がかかります。</>,
  },
  {
  ...
  {
    q: "軽自動車の車検はいくらですか？",
    a: <>軽自動車の車検費用は、法定費用（{gl("jibaiseki", "自賠責保険")}17,540円＋{gl("weight-tax", "重量税")}6,600円＋{gl("stamp-fee", "印紙代")}1,400円）の約25,500円に加え、{gl("inspection-fee", "車検基本料")}10,000〜25,000円が必要です。合計で約36,000〜66,000円が目安ですが、部品交換が必要な場合はさらに費用がかかります。</>,
  },
  {
    q: "ディーラー車検はなぜ高いのですか？",
  ...
  },
  {
    ...
    a: <>{gl("brake-pad-front", "ブレーキパッド交換")}（フロント片側）の費用は、軽自動車で8,000〜18,000円、小型車で10,000〜25,000円、中型車で12,000〜30,000円が相場です。工賃込みの価格で、純正品かアフターパーツかでも金額が変わります。</>,
  },
];
