    ...
    <form onSubmit={handleSubmit} className="animate-fade-in-up" aria-label="見積もり入力フォーム">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-1 text-sm text-slate-500 hover:text-[#0d7377] transition-colors cursor-pointer"
        >
      ...
      <button
        type="button"
        onClick={onAddItem}
        className="mt-3 w-full py-2.5 rounded-lg border border-dashed border-slate-300 text-sm text-slate-500 hover:border-[#0d7377] hover:text-[#0d7377] hover:bg-[#f0fafa] transition-all cursor-pointer"
      >
        + 項目を追加
      </button>
      ...
      <div className="mt-6 hidden sm:block">
        <button
          ...
          className="w-full py-3.5 rounded-xl bg-[#0d7377] text-white font-bold text-sm shadow-sm hover:bg-[#095b5e] disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
        >
          {hasValidItem
      ...
      <div className="fixed bottom-0 left-0 right-0 p-3 pb-[calc(0.75rem+env(safe-area-inset-bottom,0px))] bg-white/95 backdrop-blur-sm border-t border-slate-200 sm:hidden z-50">
        <button
          type="submit"
          disabled={!hasValidItem}
          className="w-full py-3.5 rounded-xl bg-[#0d7377] text-white font-bold text-sm disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
        >
          {hasValidItem
            ? `${validCount}件の見積もりを診断する`
            : "項目と金額を入力してください"}
